...
function useWebRTC({ peerId, displayName, addLog, onHandshakeReceived, onMeshMessageReceived, onFileReceived }) {
    ...
    const cleanupPeerConnection = (targetPeerId) => {
    ...
    const setupDataChannel = (targetPeerId, channel, getHandshakeData) => {
        dataChannels.current.set(targetPeerId, channel);
        channel.onopen = async () => {
            addLog('WebRTC', `DataChannel opened with peer ${targetPeerId}`);
            if (window.api)
                window.api.webrtcStatus(targetPeerId, 'connected');
            try {
        ...
        channel.onclose = () => {
            ...
            if (window.api)
                window.api.webrtcStatus(targetPeerId, 'offline');
    ...
    const initiateWebRTCConnection = async (peer, getHandshakeData) => {
        try {
    ...
    const handleIncomingSignal = async (payload, getHandshakeData) => {
    ...
    const webrtcSendPacket = (peerId, message, encryptFn) => {
    ...
    const webrtcSendFile = (targetPeerId, file, onFileSent) => {
        const channel = dataChannels.current.get(targetPeerId);
        if (!channel || channel.readyState !== 'open') {
